...
class StableDiffusion3Pipeline(DiffusionPipeline, SD3LoraLoaderMixin, FromSingleFileMixin, SD3IPAdapterMixin):
    ...
    # Compress the "latent" value, with scale = 100 indicating the retention of two decimal places.
    def _compress_latents(self, latents, scale=100):
        return torch.round(latents * scale) / scale
    ...
    def __call__(
        ...
        compress_at_step: Optional[int] = None,
        compress_scale: int = 100,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # At the specified compression steps, perform latent compression
                if compress_at_step is not None and i == compress_at_step:
                    latents = self._compress_latents(latents, scale=compress_scale)
